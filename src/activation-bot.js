...
class ActivationBot {
  constructor() {
    ...
    this.conversationService = new ConversationService(
      this.botActivationArticleService,
    );
    ...
    this.config = {
      targetChannelId:
        process.env.ACTIVATION_TARGET_CHANNEL_ID || "1206794672375205939", // Channel where EarthRovers threads are created
      refreshInterval:
        parseInt(process.env.ACTIVATION_REFRESH_INTERVAL) ||
        24 * 60 * 60 * 1000, // 24 hours
      supportRoleId:
        process.env.ACTIVATION_SUPPORT_ROLE_ID || "1217016478193422406", // EarthRovers support team role ID

      // Rate limiting configuration
      rateLimits: {
        maxQueriesPerMinute:
          parseInt(process.env.ACTIVATION_MAX_QUERIES_PER_MINUTE) || 5, // Maximum AI queries per user per minute
        maxQueriesPerHour:
          parseInt(process.env.ACTIVATION_MAX_QUERIES_PER_HOUR) || 30, // Maximum AI queries per user per hour
        cooldownSeconds:
          parseInt(process.env.ACTIVATION_COOLDOWN_SECONDS) || 10, // Cooldown between messages from same user
        windowSizeMinutes:
          parseInt(process.env.ACTIVATION_WINDOW_SIZE_MINUTES) || 1, // Time window for rate limit tracking (minutes)
        windowSizeHours:
          parseInt(process.env.ACTIVATION_WINDOW_SIZE_HOURS) || 1, // Time window for hourly rate limit tracking
        cleanupIntervalMinutes:
          parseInt(process.env.ACTIVATION_CLEANUP_INTERVAL_MINUTES) || 5, // How often to clean up old rate limit data
      },
    };
  ...
  async shutdown() {
    ...
    console.log(
      `📊 Escalation stats: ${stats.escalatedThreadsCount} threads escalated, ${stats.processedThreadsCount} threads processed`,
    );

    // Show rate limiting statistics before shutdown
    const rateLimitStats = this.getRateLimitStats();
    console.log(
      `📊 Rate limit stats: ${rateLimitStats.totalTrackedUsers} users tracked, ${rateLimitStats.activeUsersThisMinute} active this minute`,
    );
  ...
  setupEventListeners() {
    this.client.once("ready", async () => {
      console.log(
        `🌍 Activation Bot is ready! Logged in as ${this.client.user.tag}`,
      );
      console.log("🤖 Specialized for Activation support");
      console.log(
        "🚫 Escalation tracking enabled - AI stops responding after support team tagged",
      );
      ...
      try {
        const commandData = Array.from(activationCommands.values()).map(
      ...
      console.log(
        "🚀 EarthRovers Bot: Initializing comprehensive knowledge base...",
      );
      console.log(
        `🎯 Target channel for EarthRovers threads: ${this.config.targetChannelId}`,
      );
      console.log(
        `👥 EarthRovers support role ID: ${this.config.supportRoleId}`,
      );
      console.log(
        `⏱️ Rate limits: ${this.config.rateLimits.maxQueriesPerMinute}/min, ${this.config.rateLimits.maxQueriesPerHour}/hour, ${this.config.rateLimits.cooldownSeconds}s cooldown`,
      );
      ...
      try {
        ...
        console.error(
          "❌ Failed to initialize EarthRovers content:",
          error.message,
        );
    ...
    this.client.on("interactionCreate", async (interaction) => {
      ...
      if (!command) {
        console.error(
          `❌ No activation command matching ${interaction.commandName} was found.`,
        );
        return;
      }
      ...
      try {
        ...
        console.error(
          `❌ Error executing activation command ${interaction.commandName}:`,
          error,
        );
  ...
  async initializeEarthRoversContent() {
    ...
    const earthRoversArticles =
      await this.botActivationArticleService.getAllEarthRoversArticles();
    ...
    if (
      ...
      console.log(
        `📊 Token Usage: ${diagnostics.tokenUsage.current}/${diagnostics.tokenUsage.maximum} (${diagnostics.tokenUsage.percentage}%)`,
      );
      console.log(
        `📄 Articles: ${diagnostics.cacheInfo.totalArticles} cached, ${diagnostics.cacheInfo.freshArticles} fresh`,
      );
      console.log(
        `⚖️ Content Limits: ${diagnostics.contentLimits.maxUrls} URLs, depth ${diagnostics.contentLimits.maxDepth}`,
      );
  ...
  async refreshEarthRoversContent() {
    ...
    const refreshedContent =
      await this.botActivationArticleService.refreshContent();

    if (
      refreshedContent &&
      refreshedContent !== "EarthRovers article content unavailable"
    ) {
      ...
      const diagnostics = this.botActivationArticleService.getDiagnostics();
      console.log(
        `📊 Updated Token Usage: ${diagnostics.tokenUsage.current}/${diagnostics.tokenUsage.maximum} (${diagnostics.tokenUsage.percentage}%)`,
      );
      console.log(
        `📄 Updated Articles: ${diagnostics.cacheInfo.totalArticles} cached, ${diagnostics.cacheInfo.freshArticles} fresh`,
      );
  ...
  isEarthRoversActivationThread(thread) {
    if (!thread || !thread.parentId) {
      console.log(
        `❌ Invalid thread object or missing parent channel:`,
        thread?.parentId,
      );
      return false;
    }
    ...
    console.log(
      `🔍 Channel check: Thread in channel ${thread.parentId} -> ${isTargetChannel ? "✅ MATCH" : "❌ NO MATCH"}`,
    );
  ...
  async handleThreadCreation(thread) {
    try {
      ...
      if (this.cachedSystemPrompt) {
        console.log(
          `✅ EarthRovers knowledge base ready for thread: ${thread.name}`,
        );
      } else {
        console.log(
          `⚠️ EarthRovers knowledge base not yet available for thread: ${thread.name}`,
        );
        console.log(
          `📚 Content will be available once initialization completes`,
        );
      }
    } catch (error) {
  ...
  async handleEarthRoversThreadMessage(message) {
    ...
    try {
      ...
      if (member && member.roles.cache.has(this.config.supportRoleId)) {
        console.log(
          `🛑 User ${username} (${userId}) is a support staff (role: ${this.config.supportRoleId}). Ignoring message.`,
        );
        return;
      }
    } catch (err) {
      console.log(
        `⚠️ Could not fetch member or roles for user ${userId}:`,
        err.message,
      );
    ...
    try {
      ...
      if (this.escalatedThreads.has(threadId)) {
        console.log(
          `🚫 Thread ${threadId} has been escalated to human support - AI will not respond`,
        );
      ...
      console.log(
        `🤖 Processing EarthRovers question from ${username} in thread: ${message.channel.name}`,
      );
      ...
      console.log(
        `📊 Rate limit: ${username} - ${userStatus.queriesThisMinute}/${this.config.rateLimits.maxQueriesPerMinute} per min, ${userStatus.queriesThisHour}/${this.config.rateLimits.maxQueriesPerHour} per hour`,
      );
      ...
      if (this.cachedSystemPrompt) {
        ...
        const conversationHistory =
          this.conversationService.getConversationHistory(userId, false);
        ...
        if (aiResponse.isValid) {
          ...
          const fallbackMessage =
            "I'm sorry, I'm having trouble processing your request right now. Please try rephrasing your question or ask for human support by typing 'talk to team'.";
        ...
        const fallbackMessage =
          "I'm currently loading my EarthRovers knowledge base. Please try again in a moment, or ask for human support by typing 'talk to team'.";
        await message.reply(fallbackMessage);

        if (typingInterval) clearInterval(typingInterval);
        console.log(
          `⚠️ System prompt unavailable for ${username}, sent fallback`,
        );
      }
    } catch (error) {
      if (typingInterval) clearInterval(typingInterval);
      ...
      try {
        const errorMessage =
          "I apologize, but I'm experiencing technical difficulties. Please try again or request human assistance by typing 'talk to team'.";
...
Respond with ONLY one word: "ESCALATE" or "CONTINUE"
      ...
      if (isEscalationRequest) {
        console.log(
          `🚨 EarthRovers human help request detected from ${message.author.username}: "${message.content}"`,
        );
      }
  ...
  async escalateToSupportTeam(message) {
    try {
      const threadId = message.channel.id;
      console.log(
        `📞 Escalating EarthRovers thread to support team for user: ${message.author.username}`,
      );

      // Mark this thread as escalated - AI will no longer respond
      this.escalatedThreads.add(threadId);
      console.log(
        `🚫 Thread ${threadId} marked as escalated - AI responses disabled`,
      );
...
(*EarthRovers AI bot will no longer respond to messages in this thread.*)`;
      ...
      console.log(
        `✅ EarthRovers support team (${this.config.supportRoleId}) tagged in thread: ${message.channel.name}`,
      );
      ...
      try {
        ...
        const fallbackMessage =
          "I'll connect you with our EarthRovers support team. Please wait for assistance.";
        await message.reply(fallbackMessage);

        // Still mark as escalated even if there was an error
        this.escalatedThreads.add(message.channel.id);
        console.log(
          `⚠️ Thread ${message.channel.id} marked as escalated despite error`,
        );
      } catch (replyError) {
        console.error(
          "🚨 Error sending escalation fallback message:",
          replyError,
        );
      }
    }
...
- Keep responses concise - only answer what was asked`;
  ...
  resetThreadEscalation(threadId) {
    ...
    if (wasEscalated) {
      console.log(
        `🔄 Escalation reset for thread: ${threadId} - AI can respond again`,
      );
    } else {
  ...
  clearAllEscalations() {
    ...
    console.log(
      `🧹 Cleared ${count} escalated threads - AI can respond in all threads again`,
    );
    return count;
  }
  ...
  isUserRateLimited(userId) {
    ...
    if (lastMessage && now - lastMessage < rateLimits.cooldownSeconds * 1000) {
      const remainingCooldown = Math.ceil(
        (rateLimits.cooldownSeconds * 1000 - (now - lastMessage)) / 1000,
      );
      return {
        limited: true,
        reason: "cooldown",
    ...
    if (recentQueries.length >= rateLimits.maxQueriesPerMinute) {
      ...
      const resetTime = Math.ceil(
        (oldestQuery + rateLimits.windowSizeMinutes * 60 * 1000 - now) / 1000,
      );
      return {
        limited: true,
        reason: "per_minute",
    ...
    if (hourlyQueries.length >= rateLimits.maxQueriesPerHour) {
      ...
      const resetTime = Math.ceil(
        (oldestQuery + rateLimits.windowSizeHours * 60 * 60 * 1000 - now) / 60,
      );
      return {
        limited: true,
        reason: "per_hour",
  ...
  recordUserQuery(userId) {
    ...
    const oneHourAgo =
      now - this.config.rateLimits.windowSizeHours * 60 * 60 * 1000;
    userLimits.queries = userLimits.queries.filter(
  ...
  getUserRateLimitStatus(userId) {
    ...
    const now = Date.now();
    const oneMinuteAgo =
      now - this.config.rateLimits.windowSizeMinutes * 60 * 1000;
    const oneHourAgo =
      now - this.config.rateLimits.windowSizeHours * 60 * 60 * 1000;

    return {
      queriesThisMinute: userLimits.queries.filter((t) => t > oneMinuteAgo)
        .length,
      queriesThisHour: userLimits.queries.filter((t) => t > oneHourAgo).length,
      lastMessage: userLimits.lastMessage
        ? new Date(userLimits.lastMessage)
        : null,
    };
  }
  ...
  cleanupRateLimitData() {
    ...
    for (const [userId, userLimits] of this.userRateLimits.entries()) {
      ...
      if (
        userLimits.queries.length === 0 &&
        userLimits.lastMessage < oneHourAgo
      ) {
    ...
    if (cleanedUsers > 0) {
      console.log(
        `🧹 Rate limit cleanup: removed ${cleanedUsers} inactive users`,
      );
    }
  }
  ...
  setupRateLimitCleanup() {
    const cleanupInterval =
      this.config.rateLimits.cleanupIntervalMinutes * 60 * 1000;
    this.rateLimitCleanupInterval = setInterval(() => {
      this.cleanupRateLimitData();
    }, cleanupInterval);

    console.log(
      `⏰ Rate limit cleanup scheduled every ${this.config.rateLimits.cleanupIntervalMinutes} minutes`,
    );
  }
  ...
  async handleRateLimitExceeded(message, rateLimitCheck) {
    ...
    console.log(
      `⚠️ Rate limit exceeded for ${username}: ${rateLimitCheck.reason} (${rateLimitCheck.current}/${rateLimitCheck.limit})`,
    );
